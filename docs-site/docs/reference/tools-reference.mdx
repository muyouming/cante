...
By default Cante enables `Read`, `Bash`, `Grep`, `Glob`, `TodoWrite`, `WebFetch`, `Edit`, `Write`, and `Agent`. `WebSearch` is added automatically when the session's provider supports native web search.
...
cante --include-tools ViewImage -p "describe the screenshots in assets/"
