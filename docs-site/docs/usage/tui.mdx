...
When the agent proposes file edits, Cante switches to a fullscreen diff view on an alternate screen. You can review the exact changes before approving. Diff views use numbered gutters, full-width added/removed bands, and `+N -M` change counts in their headers.
...
Cante includes a theme system for consistent styling. Choose a theme through the theme dialog.
