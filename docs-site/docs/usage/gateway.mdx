...
This reads `~/.cante/channels.json`, connects to every platform you've enabled, and starts listening for messages.
...
| `--offline-model` | — | Path to a local GGUF file. Starts a shared `llama serve` router before accepting messages and routes every conversation through it (forces `--provider local --model local`) |

:::tip
The defaults assume a local llama.cpp server or router. To use a hosted provider, pass `--provider` and `--model` explicitly (e.g. `cante gateway --provider anthropic --model claude-sonnet-5`).
:::
