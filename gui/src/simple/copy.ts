...
export const WIZARD = {
  ...
  welcomeButton: "开始检查",
    /**
     * 「检查你的电脑」兜底分支上的复制按钮（#175 走查抓到）：那一支原来只有两行字，
     * 而文案里写着「点「复制详情」…」—— 屏幕上却没有那个按钮，她会去找一个不存在的东西 ✗。
     * 这一支正是 Windows 上「文件在、起不来」（杀软拦、缺运行库）会走到的形态。
     */
    copyDetailLabel: "复制详情（给技术同事看）",
    copyDetailFailed: "没复制成功。可以按住鼠标把上面的字选出来，再按 Ctrl/Cmd+C 复制。",
  checkTitle: "检查你的电脑",
  checking: "正在检查，请稍等……",
  readyTitle: "已经就绪",
...
export const WIZARD_HEALTH = {
  ...
  engine:
    "这台电脑上还缺一个干活需要的程序。点「复制详情」发给帮你装这个软件的人；装好之后点「重新检查」。",
...
export const ERROR_VIEW = {
  ...
  /**
   * 复制出去的**技术详情**的头几行（#175 走查：原来只有程序那一行原文，42 字节 ✗，
   * 同事拿到会问"这是啥" ✓）。这几行都在手边：哪个程序、哪件事、什么时候。
   */
  detailHeader: "Cante 出错详情",
  detailTask: "任务：",
  detailWhen: "时间：",
} as const;
...
export const ERRORS = {
  ...
  modelHow:
    "点「重试」；如果一直这样，说明这台电脑还没准备好。点「复制详情」发给帮你装这个软件的人。",
...
// #192 A — 「先给我看一眼再交上去」。
//
// ROADMAP §6.5 的调研结论：这是她（画像：王姐，45 岁，行政/财务）的真实动作，
// 判定是「要在界面上更显眼，而不是藏在按钮里」；而画像第 2 条说她放弃产品的
// 原因是「怕弄坏东西」。所以这一段不能只是页脚里第三个按钮：它单独成块、固定
// 摆在页脚上方（一打开确认页就看得见，不用先滚到底），并用**她的话**说清「只看
// 不动」。
//
// 用词取她的原话「先给我看一眼」，不用「试跑」——那是个她不会说的词。
// ---------------------------------------------------------------------------
export const TRY_FIRST = {
  /** 这一块的标题：她的原话，点明这一步不走。 */
  heading: "先给我看一眼（只看不动）",
  /** 一句平实中文：说清这一步做什么、什么不会发生。 */
  hint: "我先只说明打算怎么做：你的文件一个字都不会改，也不会多出文件。看完你再决定要不要开始。",
  /** 按钮：点它就是「先看一眼」，不会开始动手。 */
  action: "先给我看一眼",
} as const;
...
export const APPROVAL = {
  ...
  /**
   * 助手那句**原话**的标题（#175 走查抓到）：它原来是原样渲染进正文的，于是在
   * 「要不要允许它继续」这一屏的正中间夹了一句英文（`Allow?`）—— 而这恰恰是她最需要
   * 看懂的一屏。原话移进「详情」，正文只留中文事实。
   */
  rawLabel: "它原话是这么说的（给技术同事看）：",
  /** 列表意外为空时也必须能作答 */
