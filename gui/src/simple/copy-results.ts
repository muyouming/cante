...
export const RESULTS = {
  ...
  // ---- 现在还在不在（四种说法，一种都不含糊） ---------------------------
  //
  // `missing` 为什么不写死原因（第 14 批，证据见 gui/docs/MISSING-FILE.md）：
  // 她的结果常常就在她自己的 U 盘 / 移动硬盘上（结果留在原文件旁边）。她把盘拔了、
  // 或以为插着，本机能看到的只有一个 bit——**路径在不在**——而「文件被删」「盘被拔」
  // 「整个文件夹被删」这三种处境下这个 bit 完全一样（都是「不在」）。实验（macOS，
  // 临时 dmg 卸载，五个现场）记在那份文档里：直接父目录那个信号**会用错**（删掉整个
  // 文件夹时它的父目录也不在，会把那种情况误报成「盘拔了」）；卷挂载根能分开，但那只
  // 在 macOS 上核过、Windows 未核，且要把它显示出来得动本轮范围外的文件。所以
  // **不新增**一个「盘不在」的自动判定——那是猜。能做的是别把话说死：把她可能遇到的
  // **两种**原因都摆出来，再给两步出路（把那个盘插回来 / 重新选一次文件），而不是
  // 只说一句「被移动或删掉了」。
  // 注意：那句「可能被移动或删掉了」必须逐字保留，它是 readability 白名单里那条
  // 被动的锚点（readability.test.ts 的 PASSIVE_ALLOW）；改掉它会红。
  presence: {
    present: "现在还在，能打开。",
    missing:
      "这个文件现在找不到了。它可能被移动或删掉了，也可能在一个现在没接上的盘上（比如 U 盘、移动硬盘）。原文件没受影响。把那个盘插回来再点一次。还不行，就打开所在文件夹找一找，或者回首页重新选一次文件。",
  ...
  actions: {
    ...
    /**
     * 「复制位置」：她说给别人听不方便（那串位置又长又绕），就一步把它放进剪贴板，
     * 她自己粘出去。复制不是发送——只放到她的剪贴板，发不发、发给谁，她自己定。
     */
    copyLocation: "复制位置",
    // 名字带上文件名：和上面两个按钮同一条道理（WINDOWS-ACCEPTANCE-19.md §3）——
    // 一屏几十行结果时，读屏要能听出这是**哪一份**的位置。
    ariaCopyLocation: (name: string): string => `复制 ${name} 的位置`,
    /** 复制成功；顺带告诉她粘到哪儿能用（她第一次用可能不知道拿它做什么）。 */
    copied: "已经复制好了。粘到微信里，或者「打开文件」的窗口里，都能用。",
    /** 复制没成功（例如系统不让写剪贴板）：如实说，并给一条她自己能走的路。 */
    copyFailed: "没能复制上。可以点「打开所在文件夹」找一找。",
    /** 文件已经不在时，「打开文件」为什么点不动。 */
 ...
 * 这条位置粘出去时长什么样。
 *
 * 界面上的机器路径一律写成正斜杠（`files.rs` 的 display 有意如此：确认页、给助手的
 * 指令、运行记录、前后对比的键都共用这一种写法），但那是**界面的**写法，不是
 * Windows 习惯的写法。她要的是把它粘到两个地方：微信里说给同事听，或者自己粘进
 * 「打开文件」那个窗口去找文件——两处都是 Windows 的地盘，用反斜杠才对。
 *
 * 所以这里只给**要复制的那一份**换写法，不动显示（分开处理）：
 *   * 认得出是 Windows 的位置（盘符开头，例如 C:/…）→ 换成反斜杠，并去掉多余分隔符；
 *   * 别的位置（苹果、Linux 上那种以 / 开头的）本身就用正斜杠，改了反而是错的，原样返回。
 *
 * 认不出、空串都原样返回，绝不猜。纯函数，没有 Solid、没有剪贴板，所以 bun test
 * 能直接钉住它（例如「不能出现连续两个分隔符」）。
 */
export function pathForClipboard(path: string): string {
