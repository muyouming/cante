...
export const wechatTableTask: TaskDef = {
  ...
  plan: TABLE_PLAN,
  risks: [
    "聊天记录里缺时间的消息，我会保留下来并标出「时间看不清」，不会丢掉。",
    "语音、图片、表情在导出的记录里通常没有文字，表格里不会有这些内容；要是这几样正是你要的，请你先转成文字再给我，我抄不进表格里。",
...
export const wechatDraftTask: TaskDef = {
  ...
  risks: [
    ...
    "语气按常见的礼貌写法，不一定是你和对方平时的说话方式，发之前请你自己读一遍。",
...
export const wechatBatchTask: TaskDef = {
  ...
  risks: [
    ...
    "草稿的语气按常见的礼貌写法，发之前请你自己读一遍；一条都不会自动发出去。",
  ],
  prompt(files: string[], instruction: string): string {
    return buildPrompt({
...
export const wechatRollcallTask: TaskDef = {
  ...
  risks: [
    ...
    "接龙里没贴到的部分我看不见：群里往上翻还有内容、或者你少选了几条，表里就不会有；请你把整段接龙贴全了再给我。",
    "真看不懂的条目我会照原样抄在表格最后，不会丢掉，也不会自己猜一个名字填上。",
  ],
  prompt(files: string[], instruction: string): string {
    return buildPrompt({
...
export const wechatMissingTask: TaskDef = {
  ...
  risks: [
    ...
    "去掉空格、全角半角算一个字这类做法会让配对更宽松；我实际用了哪几条会在结果最前面写清，你一看就知道我是怎么对的。",
    "接龙里没贴到的部分我看不见：群里往上翻还有内容、或者你少选了几条，就会有人被误判成没交；请你先确认接龙是完整的，再拿结果去催人。",
  ],
  prompt(files: string[], instruction: string): string {
    return buildPrompt({
