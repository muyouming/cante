...
export const NOTICE = {
   ...
   *
   * 那一句关于备份的话必须和实际做法一致（见 `gui/docs/BACKUP-GROWTH.md`）：
   * 备份目录是按运行记录留的，记录一被 200 条上限挤掉，对应的备份目录也会被清走
   * （`files.rs` 的 `cleanup_orphan_runs`）。所以这里**不能说**「备份可能还在」——
   * 那会让她照着去找一个已经不存在的东西（就是 #256 修的那类「说了不做」）。
   * 给的出路都是真有的：去「我做的结果」看现在的文件；更早的备份已经为了让电脑
   * 不被占满而清掉了，真想要回动手前的样子，可以请懂电脑的同事来帮忙。
   */
  undoNothingHow:
    "你可以打开「我做的结果」，看一眼这次做出来的文件还在不在、是不是你要的。这台电脑只留最近这些次的备份，更早的会清掉、腾出地方。想变回动手前的样子，可以请懂电脑的同事来帮忙。",
