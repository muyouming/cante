// 造「服务方没了」的几种现场用的假服务方（只在这一台验收机上跑，不进产品）。
//
...
//   * proxy407 —— 每个请求都回 407 + Proxy-Authenticate（公司代理**要你先登录/验证**）
//                 → 上一轮没覆盖的那种「代理要验证」。注意 407 与 403 是**不同的错法**：
//                 403 是「就是不让过」，407 是「你先证明你是谁」。屏幕上不该把任何一种
//                 说成文件问题，但产品要不要给它一条不同的出路，是产品决定（见报告）。
//
// 注意：**真拔网线**那种现场不在这里（它由 Windows 防火墙按远端端口挡出站，见 run-offline.ps1
// 的 -Scenario cutfw）—— 那种现场才是「不是我们摆的假服务方」，所以刻意不放进这个文件。
//
// 它**只**实现 pi 真正会发的两条路径：GET /v1/models 与 POST /v1/chat/completions（SSE）。
// 用法： node relay.mjs <dead|cut|forbid|proxy407> <port> [--marker <文件>]
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (mode === "proxy407") {
      // 公司代理要你先登录/验证：407 + Proxy-Authenticate。
      // 这是与 403 **不同**的一种错法：不是「不放行」，是「还没证明你是谁」。
      // 正文照真实代理网关常见的英文说明写（她要看到的是产品翻出来的中文）。
      const payload = JSON.stringify({
        error: {
          message: "Proxy Authentication Required: please sign in to the corporate proxy",
          type: "proxy_error",
          code: 407,
        },
      });
