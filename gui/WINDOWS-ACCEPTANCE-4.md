...
- **全量跑完了一次**（计划任务触发，15:47:16 → 16:07:44，约 20 分钟）：26 张卡，
  **通过 19 / 停下问问题 3 / 失败 4**，报告 30,019 字节，日志与 `history.log` 都写上了（第 5.3 节）。
- **顺带抓到两件事**（都记在第 5.5 节，都没改产品代码）：① 我第一版的 runner 有个 PowerShell
  的坑，导致"报告出来了、但日志/历史没写"（已修并重跑验证）；② 文书那几张卡会把结果写到
  **真实桌面**上，普查因此把它们记成"没有产出"，桌面也被写进了 4–6 个文件（我清掉了）。
...
**跑完的结果**（两次全量跑，都是计划任务触发的，`-Cards` 未加 → 全部卡）：

| | 第一次（15:23→15:40） | 第二次（15:47→16:07，runner 修好之后） |
| --- | --- | --- |
| 卡片结论 | 26 张：通过 18 / 停下问问题 5 / **失败 3** | 26 张：通过 19 / 停下问问题 3 / **失败 4** |
| 报告 | `report-20260917.md` 30,620 字节 | 30,019 字节 |
| 任务 Last Result | 1 | 1 |
| runner 自己的日志 | **只有表头**（见 5.5 的坑） | 完整输出 + `结束：2026-09-17T16:07:44；退出码 1；报告 …` |
| `history.log` | 没写上 | **写上了一行** |

`history.log` 现在的内容（原样）：

```
2026-09-17T15:21:47  退出码=0  7,177 字节  C:\cante-sweep\report-20260917.md
2026-09-17T15:47:16  退出码=1  30,019 字节  C:\cante-sweep\report-20260917.md
```

（第一行是我手工跑一张卡那次，第二行才是计划任务那次。）

**两张报告的结论不完全一样**：第二次多失败一张（`doc.leave[原话]`），第一次它是"停下问问题"。
这类卡本来就会因为模型随机性在"问问题/失败"之间摆动，**看报告的变化趋势，别拿单次失败当回归**。
...
### 5.5 顺带抓到的两件事（都没改产品代码）

**（一）我第一版 runner 自己的坑：报告出来了，日志和历史没写。**
第一次计划任务跑完（15:23→15:40），报告确实出来了（30,620 字节、26 张卡），但：
`logs/sweep-20260917.log` **只有 6 行表头**、`history.log` **没有新增行**、任务 `LastTaskResult = 1`。
现象跟任务里预警的"任务跑了但什么也没写"一模一样，所以值得记下来：

原因是我在脚本里写了 `$ErrorActionPreference = 'Stop'`，而 PowerShell 5.1 下
**原生命令往 stderr 写东西**（普查会往 stderr 写进度）会被当成终止性错误（NativeCommandError）——
脚本当场死掉，**而 wsl.exe 那边的子进程会继续把普查跑完**。所以看到的是"报告有、日志没"。
修法：采集 `wsl.exe` 输出那一段活在 `Stop` 之外（脚本里有注释写明原因）。
修完重跑一遍，日志与 `history.log` 都写上了（见 5.3 的表）。

**（二）文书那几张卡会把文件写到真实桌面上，普查的隔离没兜住。**
两次全量跑里的失败卡都是同一族：`doc.notice[...信息齐全]`、`doc.leave[...]`、`doc.report[...信息齐全]`。
报告里它们的结论都是"没有产出"，而同一行"最后一句"写着文件落在了
`C:\Users\<用户>\Desktop\…`（Linux 侧看到的是 `/mnt/c/<用户>/Desktop/…`）。
也就是说：**助手按卡片提示词把结果写到了真实桌面，而普查只在自己的按卡目录里找产出**，
于是记成"没有产出"。

副作用是真的：真实桌面被写进了文件（第一次 4 个、第二次 6 个，`.docx` / `.txt` 都有），
我把它们全删了（桌面现在只剩原有的几个快捷方式）。

结论：`gui/scripts/sweep/` 的隔离（每张卡一个目录 + `HOME` 指向临时目录）
**挡不住提示词里写死的绝对路径**。对每周普查的影响有两层：
① 这几张卡每周都会假失败一次（不是产品回归，看报告时别当成红灯）；
② 会在真实桌面留垃圾。
**建议单独开一张单子**，两条路：要么普查把这类"写桌面"的产出重定向回工作目录，
要么让卡片的"结果放哪"由调用方指定（而不是写死桌面）。
...
5. **计划任务的全量跑**：验过了（第 5.3 节两次全量跑都有证据）。**但两次结论不一样**
   （失败 3 → 4），所以"这一周的 4 张失败"能不能算回归，需要人看报告里的证据段。
...
21-scheduled-run1-report.md      计划任务第一次跑出来的报告（runner 有坑那次）
22-history-before-fix.log        修之前的 history.log（只有手工跑那一行）
23-desktop-litter.txt            普查写到真实桌面上的文件（已清掉）
24-scheduled-run2.txt            第二次（修好后）的结论行 / 失败卡 / 桌面产物
25-scheduled-run2-report.md      第二次的全量报告
26-history-after-fix.log         修之后的 history.log（计划任务那一行在了）
```
