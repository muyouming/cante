...
## 追加（第 9 批）：为什么上一次「标题探针」量出 0/12 —— **量错了产物**，并给出可复用的核对步骤

### 结论
上一轮的 0/12 **说明不了界面**：被测产物**不含**当时要检的文案。而且**我后来又查错了一次**，
两次都栽在同一件事上：**没先确认"我在看的是它真的产出的那个东西吗"**。
...
# 要挑文案时，**去源码里确认它到底在哪个文件**（copy-*.ts / *.tsx），别猜

# ③ 这个文案**在不在当前 main 上**（不在就说明它还没合并，False 是正常的）
git -C <工作树> log --oneline -1
grep -c "<新文案>" gui/src/simple/copy-*.ts
```

### 判据（写进 `VERIFICATION-MAP.md` 的那一类）
**"我看了产物"不等于"我看了对的那个产物"**。说"验证通过/失败"之前，先回答：
- 这个文件是**这一轮**编出来的吗（时间）；
- 我要找的字符串**真的属于这个文件**吗（源码在哪）；
- 这个字符串**在当前 main 上存在**吗（还没合并就必然 False）。
