...
## 9. 结论（2026-09-17 收口）：**"Windows 上能不能干活"这个问题已经有答案了**

### 9.1 现在**确定**的事实（每条都有真机证据，不是推断）

| 事实 | 证据 |
| --- | --- |
| 应用在真 Windows + 真 WebView2 上**能装能开**，且就是**中文简单模式** | `gui/WINDOWS-ACCEPTANCE-1/2/3.md`；UI Automation 读到的窗口文字 |
| 装完目录**干净**（主程序 + 表格/PDF 两个工具 + 卸载程序，无 `.d`、无 0 字节垃圾） | `gui/WINDOWS-ACCEPTANCE-3.md`（10 → 5 个文件）；正式产物复验见 `-6.md`（验收中） |
| **不用 WSL**、只用应用自带的桥 + 原生 pi，**两条真实任务都跑到 `Completed`**，其中一条用的是**产品自己拼出来的那段指令**（2,636 字） | `gui/WINDOWS-ACCEPTANCE-5.md`（48.5 秒 / 11 次工具调用 / 0 错误）+ `gui/scripts/windows/bridge-e2e.py` |
| 审批闸门在桥这条路**能用**（拒绝时命令真不执行；同一轮多个调用合成一条；部分允许部分拒绝） | `gui/docs/BRIDGE-gate.md` |
| 上游 `serve --ws` **机制成立**（一帧 = 一条 op/event、能建会话、断开不杀守护进程、**服务端强制 loopback**） | `gui/docs/SPIKE-ws-transport.md` |
| **守护进程源码不在本仓库** → 想让上游出 Windows 构建，连补丁都做不了 | 全仓库只有两个 `main.rs`、没有根 `Cargo.toml` |

### 9.2 三条路的**代价**（不再是"能不能"，而是"要付什么"）

| 路 | 机制上 | 要付的代价 | 适合谁 |
| --- | --- | --- | --- |
| **A. WSL** | 已验证 ✓ | 对王姐不可接受（要她装 WSL ✗）；可作**我们自己的验收手段** ✓ | 我们自己 / 技术同事 |
| **B. 等上游** | 无法推动 ✗ | 等于**没有 Windows 版** | 没人 |
| **C. 桥 + 原生 pi** | **已端到端跑通** ✓✓ | ① 要把 pi 一起装进用户机器（分发、升级、**许可证**）；② 会话持久化、用量、多窗口还没做；③ 我们从此要跟 pi 的版本与行为走 | 想尽快让 Windows 用户真能用 |
| **C′. 连一台跑在别处的 `serve`（含 `ws://`）** | 机制成立 ✓ | 省掉协议翻译，但**欠下"谁启动/停止守护进程"与 `C:\` ↔ `/mnt/c/` 路径空间** ✗ → **不是更便宜，是另一组问题** | 数据留在**另一台机器**上的场景（也许对合规更友好 ✓） |

### 9.3 我的建议（作为 owner）

1. **已发布的产品维持现状** ✓：Windows 上能装能开、向导会说清缺什么、给技术同事一页可照做的步骤。"能装但干不了活"这件事**必须被如实说出来**（这是产品律，不是权宜之计 ✓）。
2. **把"让 Windows 用户真能干活"单独立项**，不要混在这次打磨里 —— 它是**分发与支持**问题（装什么、谁升级、许可证、出错找谁），不是界面问题 ✗。C 路线的机制部分**已经不需要再验证**了 ✓。
3. **上 C 之前必须先问清的三个问题**（都还没答案，且都不是技术问题）：
   - **许可证**：把 pi 作为我们安装包的一部分分发，允许吗？我们的协议适配层要开源到什么程度？
   - **升级**：pi 升级后行为变了，谁负责回归（我们的普查脚本能不能覆盖 ✓）？
   - **信任**：用户机器上多了一个我们能驱动、她不知道的东西 —— 隐私面板要不要**也**把它说清楚（说清"这台电脑上有个执行组件"，但不出现术语 ✓）？
4. **C′ 不要现在就做** ✓：它省下的工作量小于它新增的运维面（多一个进程、多一套路径映射），除非出现"必须把数据留在另一台机器"的真实需求 ✓。

## 10. 决定（2026-09-17）：**走 C —— 执行组件随安装包一起发，她永远不需要知道它存在**

### 10.1 判据只有一个：**哪条路让她这边零手工**

王姐 45 岁、行政/财务、只会复制粘贴和求和。所以真正的判据不是"哪条路工程上更漂亮" ✗，而是
**"她那边要付出什么"** ✓：

| 路 | **她要付出的** | 我们的代价 | 结论 |
| --- | --- | --- | --- |
| A. WSL | **要她装 WSL** ✗（等于要她做程序员的事） | 小（已验证 ✓） | **排除**（只留作我们自己的验收手段 ✓） |
| B. 等上游 | 永远用不了 ✗ | 零 | **排除** |
| **C. 组件随包发** | **一次安装，双击就用** ✓✓ | 打包 + 许可说明 + 版本钉住（都是我们的事 ✓） | **采用** ✓ |
| C′. 连另一台机器 | 要她理解"另一台机器"、还要有人先把它开起来 ✗ | 少一点协议翻译，多一套运维 | **不做**（除非将来真出现"数据必须留在别的机器"的需求 ✓） |

**一句话**：C 的所有代价都落在我们这边，A 和 C′ 的代价落在她那边。**那就 C。**

### 10.2 顺手解决了三个悬着的问题

1. **升级谁负责 → 她不需要管** ✓：组件**由我们钉住一个版本、随我们的版本一起升级** ✗ 她自己升不了它 ✓
   → 不存在"她那边的组件偷偷变了"这种事 ✗；我们每次发版用 **每周自动普查 + 逐卡普查** 验一遍 ✓。
   （这比"跟上游版本走"简单得多 ✓，也是唯一对她安全的做法 ✓。）
2. **许可证 → 已查实可分发** ✓（见 §9 与 `gui/THIRD-PARTY-LICENSES.md`：全部宽松或弱 copyleft ✓）。
   配套：**许可说明随软件一起给**（不靠网页 ✓ —— 她不会去网页找 ✓），入口全中文 ✓。
3. **信任 → 隐私面板说一句人话** ✓：她不需要知道架构 ✓，但**"这台电脑上有什么在替我干活"必须答得出来** ✓
   （而且那句话必须与真实行为一致 ✓ —— 用测试锁住 ✓，见 §3.6）。

### 10.3 她的体验长什么样（这就是验收标准）

1. 下载一个安装包 → **双击 → 下一步 → 完成** ✓（不装 WSL ✗、不装运行时 ✗、不开终端 ✗、不问任何技术问题 ✓）；
2. 打开 → 向导说清"能做什么、动手前会问你、原文件不会被乱动" ✓ → 首页 → 点一张卡 → **真的出一份文件** ✓；
3. 出问题时，向导/界面上是**中文人话 + 一个具体下一步** ✓（例如杀毒软件把它拦了 ✓ —— 这在 Windows 上很现实 ✓，
   她的 Defender 会拦未签名的程序 ✓）；
4. 「关于」里能看见"这个软件用了哪些别人的东西 + 它们的许可" ✓（她的法务同事也可能来问 ✓）。

**代价要说清**：安装包会从 3.3 MB 变成**上百 MB** ✗（要带一个运行时与执行组件 ✓）。
接受这个代价 ✓ —— 因为"**装完就能用**"对她的价值远大于"下载小" ✓；而且**不在她机器上首次运行时下载** ✓
（办公室 wifi 不稳是常态 ✗，下载失败比包大糟糕得多 ✗）。
