...
| **「服务方真的断了」的六种错法**（`wire` 真防火墙挡出站 / `proxy407` / `dead` / `cut` / `forbid` / `busy` 服务方忙·用完 ✓） | `gui\scripts\windows\run-offline.ps1 -Scenario wire`（或 proxy407 / dead / cut / forbid / busy；`-Exe` 指装机版就验装机版，不给就验工作树构建 ✓） | **反向判据** ✓：同一条连接「加规则前 CONNECTED (exit 0) / 加规则后 REFUSED (exit 1)」= 现场真被按住 ✓；她看到的**屏幕原文** ✓；原文件 sha256 一致 ✓；**开跑前先清残留 + 跑完删规则 + 删后 `ABSENT` 复查** ✓；`busy` 另有一段**脚本级判据**（期望文案从 `copy-service.ts` 取 + 6 个零术语词逐词扫，不对退 3 ✓）| 真把网卡禁用 ✗；真企业代理 ✗（这台机器上没有 ✓）；**五种旧现场的屏幕文案仍是人眼对照报告 ✓（只有 `busy` 那一场是脚本自动判的 ✓）** |
