...
| **周度普查报告的「原始输出」是可读中文**（r18 ✓） | 手工等价调用 `gui\scripts\windows\weekly-sweep.ps1`（WSL 坏掉时必然走的那条路 ✓） | 报告「原始输出」一节的**逐字中文** ✓（不再是把 UTF-16LE 按 GBK 解出来的乱码 + NUL ✓）；改的三个文件都过 PS AST 解析 ✓；见 `gui/WINDOWS-ACCEPTANCE-30.md` ✓ | **WSL 健康时**那条路 ✗（本机装不了 WSL ✓）；`-d` 指错发行版时报错花屏 ✗；计划任务本身没触发 ✗ |
...
| **GBK(936) 的 CSV 交进来时，她看到的是人话**（#279 ✓） | `gui\scripts\windows\accept-gbk-csv.ps1`（装完的机器上 `-Exe` 指安装目录里那个 ✓；`-Cleanup` 跑完删临时目录 ✓） | 用代码页 **936 显式**造一份 GBK 的 CSV ✓（**不靠宿主机的 ANSI 代码页** → 非 936 的机器上也造得出真样本 ✓，并打印头 12 字节 `D0 D5 C3 FB …` + 「按 UTF-8 读不下来」当输入证据 ✓）→ 真跑 `cante-sheets.exe write 结果.xlsx 那份.csv` → 逐条对：**退出 2 ✓ / 没有半成品落盘 ✓ / stderr 逐字全中文（合法 UTF-8、无 U+FFFD、无 `?`）✓ / 含「另存为」那一步 ✓ / 不含 `stream`·`valid`·`utf-8` ✓ / 路径外只剩 `Excel`·`WPS`·`xlsx`·`csv` ✓**；汇总「通过 N / N」，**0=通过 / 2=环境 / 3=产品**（照上面那套约定 ✓）；报告落在 `<BaseDir>\gbk-csv-report.txt` ✓。**出题人自己也验过它会红** ✓：拿 `cante-pdf.exe` 当产物 → 10 条里 2 条红、退出 3 ✓（这些检查真的会咬住东西 ✗）| **窗口那一侧** ✗（她在界面上看到的那一屏要 RDP 会话才看得到 ✓）；GBK 的 **xlsx**、UTF-16 的 CSV、WPS 自己的格式 ✗（#279 那一句是共用的 ✓，但只有 UTF-8 那条另有两组单测盯着 ✓）；**产物是不是最新源码编的** ✗ —— 脚本只报产物时间与字节，时间戳要人对着提交核（#247 的教训，**不做就等于没验** ✗）；`cante-sheets` 本身没编出来时它**如实退 2**，不许写成 ✓ |
| **「服务方真的断了」的七种错法**（`wire` 真防火墙挡出站 / `proxy407` / `dead` / `cut` / `forbid` / `busy` 假服务方忙·用完 / `realbusy` **真服务方上真坏的那条路** ✓） | `gui\scripts\windows\run-offline.ps1 -Scenario wire`（或 proxy407 / dead / cut / forbid / busy / realbusy；`-Exe` 指装机版就验装机版，不给就验工作树构建 ✓） | **反向判据** ✓：同一条连接「加规则前 CONNECTED (exit 0) / 加规则后 REFUSED (exit 1)」= 现场真被按住 ✓；她看到的**屏幕原文** ✓；原文件 sha256 一致 ✓；**开跑前先清残留 + 跑完删规则 + 删后 `ABSENT` 复查** ✓；`busy` 与 `realbusy` 另有一段**脚本级判据**（期望文案从 `copy-service.ts` 取 + 6 个零术语词逐词扫，不对退 3 ✓），`realbusy` 还会在页面 DOM 里查真网关回的原文当**正证** ✓| 真把网卡禁用 ✗；真企业代理 ✗（这台机器上没有 ✓）；**五种旧现场的屏幕文案仍是人眼对照报告 ✓（只有 `busy`/`realbusy` 两场是脚本自动判的 ✓）** |
...
| **「我做的结果」里每行的「复制位置」真点一下**（#290） | `gui\scripts\windows\accept-copy-location.ps1` | 铺 3 行 → **真鼠标**点第一行 + UIA 点三行 → `Get-Clipboard` 与那一行**逐字相等** ✓（盘符开头、反斜杠、`.xlsx` 收尾）、三行互不相同 ✓、按钮高 49px ✓、出现「已经复制好了」且无「没能复制上」、面板不关 ✓；自己铺行、自己还原 `runs.json` ✓；退出码 **0/2/3** ✓ | 真键盘（Tab/Enter）✗（合成键在 WebView2 上不可靠，见验收 26 §7）；显示缩放 ≠ 100% ✗；**装机版**那份 `cante-gui.exe` ✗；粘到微信后的观感 ✗；跑完当屏 ResultCard 上那份（#283）✗ |
...
| **读屏「实际念出来」的是什么**（比 UIA 名字更强 ✓） | `gui\scripts\windows\a11y\` 下的探针（`narrate-script.ps1` / `probe-narrator-*` / `probe-tts-duration` / `probe-view-counts` ✓） | **ETW 抓到 Narrator 真念出来的 `SpokenText`** ✓（例：`「打开 结果_挑出华东区-1.xlsx, 按钮,」` ✓）→ #230 的修复在**读屏嘴里**确认生效 ✓（12 条按钮句各带文件名 ✓）；**纠正了一条我们自己的高估** ✓：`ListItem` **不可 Tab** → 读屏**根本不念** ✓（**只有 Tab 到得了的才会被念** ✓）；`RawView` / `ControlView` 两个视图的对照 ✓ | **真的出声、她听到的是什么** ✗（这台机器**没有声卡** ✓ —— 验的是**朗读引擎的输入** ✓，不是音频 ✓）；**环境噪音**（画面扫描 / 输入法状态 / 窗口元素的名字）**不是我们的界面** ✗ |
