...
| **「服务方真的断了」的五种错法**（`wire` 真防火墙挡出站 / `proxy407` / `dead` / `cut` / `forbid` ✓） | `gui\scripts\windows\run-offline.ps1 -Scenario wire`（或 proxy407 / dead / cut / forbid） | **反向判据** ✓：同一条连接「加规则前 CONNECTED (exit 0) / 加规则后 REFUSED (exit 1)」= 现场真被按住 ✓；她看到的**屏幕原文** ✓；原文件 sha256 一致 ✓；**开跑前先清残留 + 跑完删规则 + 删后 `ABSENT` 复查** ✓ | 真把网卡禁用 ✗；真企业代理 ✗（这台机器上没有 ✓）|
