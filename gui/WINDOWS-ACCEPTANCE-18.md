...
1. **本文件（agent 自己写的 ✓，更完整 ✓）** —— 它多做了两件我没想到的事：
   ① 用**等价视口 `1024x608`** 去量「125% 会得到的视口」（并明确标成 ⚠️ **近似** ✓，不冒充真机缩放 ✓）；
   ② **一行命令真跑通了** ✓。
2. **`#225` 里那份「抢救版」** ✓ —— 那个 agent 后来**死在自己的长命令上** ✗
   （转录停 22 分钟 ✓、机器上**没有任何 `cante` 进程** ✓、报告当时没写出来 ✗），
   我从它的**会话转录 + 磁盘残留产物**里抢救出证据先记了一版 ✓。

两版的**结论一致** ✓：**这台机器改不了缩放**（`rc=87 ERROR_INVALID_PARAMETER` ✓）、
**当前缩放下的数字是真的** ✓、**125%/150% 的真机版面仍然没有证据** ✗。
本文件是三份脚本之外**多带的两个脚本**的出处 ✓（它在 `scale/` 之外又写了 `measure-scale-viewport.ps1` /
`scale-probe.ps1` / `scroll-reach-scale.ps1` ✓）。
